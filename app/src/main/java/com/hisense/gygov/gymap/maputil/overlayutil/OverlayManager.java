package com.hisense.gygov.gymap.maputil.overlayutil;
...
public abstract class OverlayManager implements OnMarkerClickListener, OnPolylineClickListener {
    ...
    /**
     * zoom with all the overlay including the current location in sight
     */
    public void zoomToSpan(MyLocationData location) {
        if (mBaiduMap == null) {
            return;
        }
        if (mOverlayList.size() > 0) {
    ...
    public void zoomToSpan(int width, int height) {
        ...
        if (mOverlayList.size() > 0) {
            ...
            LatLngBounds latlngBounds = builder.build();
            MapStatusUpdate u = MapStatusUpdateFactory.newLatLngBounds(latlngBounds, width, height);
            mBaiduMap.animateMapStatus(u);
        }
    }
